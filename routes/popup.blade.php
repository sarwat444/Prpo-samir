                     ...
                     <div class="col-md-7">
                       <select name="task_responsible"  data-name="task_responsible" class="form-control custom-select target">
                        @foreach ($users as $key => $user)
                          <option value="{{$user->id}}" @if($task->task_responsible == $user->id ) selected='selected' @endif > {{$user->user_name }} </option>
                        @endforeach
                     </select>
                  </div>
                  ...
                  <div class="col-md-7">
                     <select id="select3" name="teams_id[]" class="target" multiple data-name="teams_id">
                                 ...
                                 @php
                                     ...
                                     <option  id="{{$key}}"  value="{{$user3->id}}" name="teams_id[]"   data-left="{{asset('public/assets/images/users/'.$user3->image)}}"  @if(in_array($user3->id , $team_ids2)) selected  @endif >    {{$user3->user_name}}</option>
                                      @endforeach
                                   @endif
                            ...
                            <select id="select5" name="guests_id[]" class="target" multiple data-name="guests_id">
                                               ...
                                               @if(!empty($users2))
                                                    @foreach ($users2 as $key => $user2)
       						                                    <option  id="{{$key}}"  value="{{$user2->id}}" data-left="{{asset('public/assets/images/users/'.$user2->image)}}"  @if(in_array($user2->id , $guest_ids2)) selected  @endif >    {{$user2->user_name}}</option>
       			                                      @endforeach
       			                                   @endif
     ...
     <div class="form-group">
             <div class="row">
                 ...
                 <div class="col-md-7">
                            ...
                            <select name="task_category_id_two" class="custom-select form-control target" data-name="task_category_id_two">
                                    <option value=""> Fachbereich2 wählen</option>
                                  @foreach ($cats as $key => $cat)
                                    <option value="{{$cat->id}}" @if($task->task_category_id_two == $cat->id ) selected @endif > {{$cat->category_name }} </option>
                                  @endforeach
                            </select>
               </select>
                 </div>
    ...
    <div class="form-group">
              ...
              <select id="select6" name="tags_id[]" class="target" multiple data-name="tags_id">
                ...
                @if(!empty($tags))
                     @foreach ($tags as $key => $tag)
                       ...
                       <option  id="{{$key}}"  value="{{$tagg->id}}"   @if(in_array($tagg->id , $tags_ids2)) selected  @endif >    {{$tag->tag_name}}</option>
                   @endforeach
                @endif
          ...
          <div class="form-group">
             <div class="row">
                  ...
                  <div class="col-md-7">
                      <textarea  class="form-control target txta" data-name="task_desc" >{{$task->task_desc}}</textarea>
     ...
     <!-- <div class="col-md-4">
          <div class="form-control">
            <a href="#" id="done" class="btn btn-success">Done</a>
            <a href="#" id="not_done" class="btn btn-info">Not Done</a>
         </div>
    </div> -->
		...
		<div class="comments">
		       ...
		       @foreach($task->comments as $comment)
		        ...
		        <div class="comment" id="com_dta{{$comment->id}}">
		            ...
		            <div class="row">
        		           <div class="col-md-1">
        		               <img class="comment_maker" src="{{asset('public/assets/images/users/'.$comment->user->image)}}" alt="test" style="width: 40px;height:40px;" />
        		           </div>
        		           <div class="col-md-5">
        		             <h5>{{ $comment->user->user_name }}  <span class="comment-date">  {{ \Carbon\Carbon::parse($comment->created_at)->diffForHumans()}}  </span></h5>
                          @if(!empty($comment->comment))
                         <p  contenteditable="false" id="comment_name{{$comment->id}}" data-id="{{$comment->id}}" class="commentt_name" > {!!$comment->comment!!}</p>
                         @endif
                         @if(!empty($comment->comment_image))
                        <p> <a href="{{asset('public/assets/images/comments/'.$comment->comment_image)}}" target="_blank"><img src="{{asset('public/assets/images/comments/'.$comment->comment_image)}}" style"width:30px;height:30px;"></a> </p>
                        @endif


                         @if(!empty($comment->comment_pdf))

                             <p> <a href="{{asset('public/assets/images/comments/'.$comment->comment_pdf)}}" target="_blank"> Open a PDF file. </a></p>
                        @endif
              ...
              <div class="row" style="margin-bottom:30px;">
                  <form method="post">
                    <div class="col-md-10">
                      <textarea id="commentbox" rows="5" class="form-control" placeholder="Kommentar einfügen ..."></textarea>
                      <button id="add_comment" class="send-comment send-comment2" ><i class="fas fa-paper-plane"></i></button>
                    </div>
                 </form>
              </div>
                 ...
                 <!--Multi select  22 -->
                          <div class="row">
                                        <div class="col-md-2 text-center">
                                           <lable class="control-label">Tags <i class="bi bi-tags"></i> </lable>
                                        </div>

                                  <div class="col-md-8">
                                     <select id="select55" name="tags[]" class="target" multiple data-name="tags[]">

                                                               @foreach($users as $user)
                						                                    <option  id="{{$user->id}}"  value="{{$user->id}}" data-left="{{asset('public/assets/images/users/'.$user->image)}}" > {{$user->first_name}}</option>
                			                                      @endforeach
                   ...
                   <label for="custom-file-upload" class="filupp">
                        <span class="filupp-file-name js-value">Bild hochladen </span>
                        <input type="file" name="comment_image" id="custom-file-upload"/>

                  </label>
                  <input type="submit" value="Bild hochladen" style="background-color:#3bebff;color:#fff;padding:8px;border-radius:5px;" class="submit_comment">

                     <div class="action_buttons">
                         @if($task->task_status == 0 || $task->task_status == 1)
                             <button class="remove-task btnn-remove btn btn-danger" > <i class="fa fa-trash"></i>  Löschen </button>
                         @else
                             <button class="unremove-task  btnn-remove btn btn-danger" ><i class="fa fa-trash"></i> Wiederherstellen</button>
                         @endif

                         @if($task->task_status == 0 || $task->task_status == 2)
                             <button class="complete_task btn-complete btn btn-success"> <i class="fa fa-check-circle"></i> Aufgabe erledigt </button>
                         @else
                             <button class="uncomplete_task btn-complete btn btn-success"><i class="fa fa-check-circle"></i> Aufgabe nicht erledigt </button>
                         @endif
                     </div>
...
/*newcalendar.appendChild(resinput);*/
