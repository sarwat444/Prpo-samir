...
class TasksController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            if (!empty($request->task_category_id)) {
                $category = !empty($request->task_category_id)
                    ? Category::where('id', $request->task_category_id)->whereNotNull('deleted_at')->first()
                    : null;
                if (!empty($category)) {
                    return redirect()->route('admin.dashboard')->with('error', 'Selected Category was deleted.');
                }
            }
            if(!empty($request->task_category_id_two))
            {
                $category2 = !empty($request->task_category_id_two)
                    ? Category::where('id', $request->task_category_id_two)->whereNotNull('deleted_at')->first()
                    : null;
                if (!empty($category2)) {
                    return redirect()->route('admin.dashboard')->with('error', 'Selected Category was deleted.');
                }
    ...
    public function showTaskData(Request $request)
    {
        ...
        $users = User::where('account_id', auth()->user()->account_id)->where('deleted', 0 )->where('role', '!=', 3)->where('status' , 0)->get();
        $users3 = User::where('account_id', auth()->user()->account_id)->where('deleted', 0 )->where('role', '!=', 3)->where('status' , 0 )->get();
        $users_gests = User::where('account_id', auth()->user()->account_id)->where('deleted', 0 )->where('status' , 0 )->whereIn('role', [1, 2, 3])->where('status' , 0)->get();
        ...
        $users2 = User::where('account_id', auth()->user()->account_id)->where('deleted', 0 )->where('role', 3)->where('status' , 0)->get();
        if (count($subtasks) > 0) {
    ...
    {
        $type = $request->type;
        $users = User::where('account_id', auth()->user()->account_id)->where('role', '!=', 3)->where('deleted', 0 )->where('status' , 0)->get();
        $users2 = User::where('account_id', auth()->user()->account_id)->where('role', 3)->where('deleted', 0 )->where('status' , 0)->get();
        $categories = Category::where('account_id', auth()->user()->account_id)->get();
        $tags = Tag::where('account_id', auth()->user()->account_id)->distinct('tag_name')->get('tag_name');
        //  dd($tags);
...
// Compress the image using Intervention Image
        $compressedImage = Image::make($full_path);
        $compressedImage->resize(800, 600, function ($constraint) {
            $constraint->aspectRatio();
            $constraint->upsize();
        });

// Generate a unique name for the compressed image
        $compressed_image_name = 'compressed_' . uniqid() . '.' . $image_ext;

        $compressedPath = public_path('uploads/images/compressed/');
        $compressedImage->save($compressedPath . $compressed_image_name);

        if (!empty($request->file('image'))) {
            $task = Task::find($request->task_id);
            // Delete the previous image if it exists
            if ($task->image) {
                $previous_image_path = public_path('uploads/images/compressed/') . $task->image;
                if (file_exists($previous_image_path)) {
                    unlink($previous_image_path);
                }
            }
            $task->image = $compressed_image_name;
            $task->save();
        }
    ...
    public  function my_posts()
    {
        ...
        $tasks = Task::with('category')->where(['task_added_by'=> Auth()->user()->id ])->get();
        $users = User::where('account_id', auth()->user()->account_id)->where('deleted', 0 )->where('status' , 0)->get();
        $status = 0 ;
        $title = 'My Posts' ;
        return view('admin.tasks.myposts',compact('tasks' ,'users' ,'status' ,'title'));
    }
    public function fillterMyposts(Request  $request)
    {
         if(!empty($request->subtask_user_id))
         {
             $tasks  = Task::with('category')->where(['task_added_by'=> $request->subtask_user_id ,'task_status' =>$request->task_status])->get() ;
         }else
         {
    ...
    {
        Task::where('id',$request->task_id)->update(['image' => null]) ;
    }
