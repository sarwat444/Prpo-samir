...
public function  filtercomments(Request $request)
{
        ...
        $comment = (request()->has('comment')) ?  $request->comment : 0;
        ...
        //comments  only
         if($comment == 1 ) {
             ...
             if (empty($start_due_date) && empty($end_due_date) && $erledigt == 0 && $seen == 0) {
                 $comments = Comment::where('account_id', auth()->user()->account_id)->where('status','!=',1)->get();
                 $notes = [];
                 foreach ($comments as $comment) {
             ...
             if (empty($start_due_date) && empty($end_due_date) && $erledigt == 1 && $seen == 1) {
                 $comments = Comment::where('done' , $erledigt)->where('status','!=',1)->get();
                 $notes = [];
                 foreach ($comments as $comment) {
                     $read_users = json_decode($comment['readby']);
                     if (!empty($comment->tags) && in_array(Auth()->user()->id, explode(',', $comment->tags))) {
             ...
             // 2 dates  comments  only
             if (!empty($start_due_date) && !empty($end_due_date) && $erledigt == 0 && $seen == 0) {

                 $comments = Comment::where('account_id', auth()->user()->account_id)->where('status','!=',1)->whereBetween('created_at', [$start_due_date, $end_due_date])->orderBy('created_at' , 'DESC')->get();
             ...
             if (!empty($start_due_date) && !empty($end_due_date) && $erledigt == 1 && $seen == 0) {
                 $comments = Comment::where('account_id', auth()->user()->account_id)->where('done', $erledigt)->where('status','!=',1)->whereBetween('created_at', [$start_due_date, $end_due_date])->get();
                 $notes = [];
                 foreach ($comments as $comment) {
                     if (!empty($comment->tags)) {
                         $tags = explode(',', $comment->tags);
             ...
             if (!empty($start_due_date) && !empty($end_due_date) && $erledigt == 0 && $seen == 1) {
                 $comments = Comment::whereBetween('created_at', [$start_due_date, $end_due_date])->where('status','!=',1)->get();
             ...
             //2 Dates  , erldgate ,  done
             if (!empty($start_due_date) && !empty($end_due_date) && $erledigt == 1 && $seen == 1) {

                 $comments = Comment::where('done', $erledigt)->where('status','!=',1)->whereBetween('created_at', [$start_due_date, $end_due_date])->get();
             ...
             if (empty($start_due_date) && empty($end_due_date) && $erledigt == 1 && $seen == 0) {
                 ...
                 $comments = Comment::where('account_id', auth()->user()->account_id)->where('status','!=',1)->where('done', $erledigt)->get();
                 $notes = [];
                 foreach ($comments as $comment) {
                     if (!empty($comment->tags)) {
                         $tags = explode(',', $comment->tags);
    ...
    public function  filterReplays(Request $request)
{
        ...
        $end_due_date = date('Y-m-d', strtotime($request->end_due_date));
        $seen = (request()->has('seen')) ?  $request->seen : 0;
        $replay= (request()->has('replay')) ?  $request->replay : 0;
        if($start_due_date == '1970-01-01'){
            $start_due_date = null ;
        }
        if($end_due_date == '1970-01-01'){
   ...
   public function update(Request $request , $id) {
    ...
    public function  filtertests(Request $request)
    {
            ...
            $start_due_date = date('Y-m-d', strtotime($request->start_due_date));
            $end_due_date = date('Y-m-d', strtotime($request->end_due_date));
             $tested_or_crested = $request->tested_or_crested;
            if($tested_or_crested == null) {
                ...
                {
                    $subtasks = SubTask::where(['tester' => auth::user()->id , 'tested' => 1 , 'subtask_status'=> 1])
                        ->orWhere(['subtask_added_by' => auth::user()->id])->where('tester', '!=', auth::user()->id)
                        ->whereBetween('assigned_at', [$start_due_date . " 00:00:00", $end_due_date . " 23:59:59"])->get();
                ...
                }else
                {
                    $subtasks = SubTask::where(['tester' => auth::user()->id])
                        ->orWhere(['subtask_added_by' => auth::user()->id])->where('tester', '!=', auth::user()->id)
                        ->whereBetween('assigned_at', [$start_due_date . " 00:00:00", $end_due_date . " 23:59:59"])->get();
                    $data = view('admin.subtasks.testfilter-done', compact('subtasks'))->render();
                    return response()->json(['options' => $data]);
                }



            }else if($tested_or_crested == 1 )
            {

                if(!empty($request->completed_checkbox) &&  $request->completed_checkbox == 1  )
                {
                    $subtasks = SubTask::with('added_by')
                        ->where(['tester' => \Illuminate\Support\Facades\Auth::user()->id])
            ...
            else if($tested_or_crested == 2 )
            {
                ...
                }else
                {
                    $subtasks = SubTask::with('added_by')
                        ...
                        ->with('task.category')
                        ->whereBetween('created_at', [$start_due_date, $end_due_date])
                        ->where(['subtask_added_by' => \Illuminate\Support\Facades\Auth::user()->id])
                        ->where('tester', '!=', auth::user()->id)
                        ->where(function ($query) {
                            $query->where('subtask_status', '!=', 1)
                                ->orWhere('tested', '!=', 1);
