    ...
    <div id="sidebarMenu" class="collapse d-lg-block sidebar collapse bg-white">
        <div id="edit_subtask">
                    ...
                    <div class="task-data">
                                                    ...
                                                    @foreach($task->un_completed_subtasks as $subtask)
                                                        <li @if($subtask->subtask_status != 0) class="task  ui-state-default completed"
                                                            ...
                                                            <input type="hidden" value="{{$subtask->id}}"
                                                                   class="testinput" data-id="{{$subtask->id}}"/>
                                                        </li>
                                                    @endforeach
                                                @endif
                                            </div>
                        ...
                        <div class="comments">
                            ...
                            <div class="row">
                                ...
                                <!--End Tags-->
                                <div class="col-md-1">
                                    <button type="button" id="add_comment" class="send-comment send-comment2  btn_1"
                                            data-task="{{$task->id}}">
            ...
            "></i>
                                            ...
                                            <div class="add_new_replay  add_new_replay{{$comment->id}}">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-10" style="padding-left:58px">
                                                        ...
                                                        <select id="replay_tags_{{$comment->id}}" name="tags[]" class="replay_tags" multiple data-name="tags[]" style="width: 100%; height: 50px !important; border: 1px solid #777 !important; margin-left: 23px !important;">
                                                            @php
                                                            ...
                                                            @foreach($users_gests as $user)
                                                                @php
                                                                    ...
                                                                    @endphp
                                                                <option value="{{$user->id}}"
                                                                        data-left="{{ !empty($user->image) ? asset('public/assets/images/users/'.$user->image) : asset('public/assets/images/default.png') }}"
                                                                        @if(in_array($user->id, $replay_users) && auth()->user()->id != $user->id || $comment->comment_added_by == $user->id && auth()->user()->id != $comment->comment_added_by) selected @endif
                                                                >{{$user->first_name}}</option>
                                                            @endforeach
                                                        </select>
                                                        <script>
                                                            $(document).ready(function() {
                                                                $('#replay_tags_{{$comment->id}}').select2();
                                                            });
                                                        </script>
                    ...
                    <div class="row">
                        <div class="col-md-5">
                            <form id="dataup" class="form" method="POST"
                                  action="{{route('admin.subtasks.store_comment')}}" enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="type" value="image">
                                <input type="hidden" name="task_id" value="{{$task->id}}">
                                <label for="custom-file-upload" class="filupp">
                                    <span class="filupp-file-name js-value">1- {{__('messages.choose_photo')}} <i
                                            class="fa fa-file"></i>  </span>
                                    <input type="file" name="comment_image" id="custom-file-upload"/>
                                </label>
                                <button type="submit" class="submit_comment"><i class="fa fa-upload"></i>
                                    2- {{__('messages.upload_photo')}}</button>
                            </form>
                        </div>
