                          ...
                          @if(!empty($tags))
                              @foreach($tags as $tag)
                                    <div class="swiper-slide cat_list" style="margin-right:5px;">
                                      <a href="{{route('admin.cat.tag.tasks',['cat_id' => $cat_id ,'tag_id' => $tag->id ,'status' => $status ])}}">  <div @if($tag->id == $tag_id ) class="category-button  filter active" @else class="category-button  filter" @endif   >
                        ...
                        <ul>
                          ...
                          @if(!empty($categories))
                              @foreach($categories as $cat)
                                 <a href="{{route('admin.cat.tasks',['cat_id' => $cat->id ,'status' => $status ])}}"><li class="category-button  filter"  data-status="{{$status}}">  {{$cat->category_name}} </li> </a>
                              @endforeach
                          @endif
                        </ul>
                    </div>
